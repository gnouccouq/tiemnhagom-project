...
async function fetchAvailableCoupons() {
    ...
    try {
        ...
        snap.forEach(doc => {
            ...
            // Không hiển thị voucher tặng người thân trong danh sách khả dụng công khai - người nhận phải nhập thủ công
            if (data.assignedBy || data.forNewCustomerOnly || (data.isAutoGenerated && !data.assignedTo)) return;
...
window.applyCoupon = async () => {
    ...
    try {
        ...
        if (couponSnap.exists()) {
            ...
            // Kiểm tra voucher dành cho người thân (không cho phép chính chủ tự áp dụng cho chính mình)
            if (data.assignedBy && auth.currentUser && data.assignedBy === auth.currentUser.uid) {
                showToast("Mã ưu đãi này là quà tặng dành cho người thân/bạn bè. Bạn hãy gửi mã cho người thân sử dụng nhé!", "error");
                appliedCoupon = null;
                renderCart();
                return;
            }
