    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            snapCoupons.forEach(doc => {
                ...
                // Bỏ qua các mã cá nhân (sinh nhật, hạng thành viên) và mã tặng người thân từ membership (chỉ hiển thị trong ví/membership của chủ sở hữu và chỉ áp dụng khi người nhận tự nhập mã)
                if (data.isAutoGenerated || data.assignedTo || data.assignedBy || data.forNewCustomerOnly) return;
                ...
                <div class="product-detail-grid">
                    ...
                    return `<div class="dynamic-membership-price" data-price="${currentPrice}"></div>`;
                        ...
                        <div class="product-description" id="product-description-container">
                            ...
                            <div class="product-description-wrapper" id="desc-wrapper">
                                <div class="product-description-content" id="desc-content">${p.description ? p.description : 'Hiện tại chưa có thông tin.'}</div>
                                <div class="desc-fade-overlay" id="desc-fade-overlay"></div>
                            </div>
                            <div class="desc-toggle-wrapper" id="desc-toggle-wrapper" style="display: none;">
                                <button type="button" class="btn-toggle-desc" id="btn-toggle-desc" onclick="window.toggleProductDescription()">
                                    <span id="desc-toggle-text">Xem thêm mô tả</span>
                                    <svg id="desc-toggle-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            // Khởi tạo tính năng Xem thêm / Thu gọn mô tả sản phẩm
...
function initProductDescriptionToggle() {
    ...
    const toggleWrapper = document.getElementById('desc-toggle-wrapper');
    ...
    if (!wrapper || !content || !toggleWrapper) return;

    const MAX_COLLAPSED_HEIGHT = 240;

    const checkHeight = () => {
        if (content.scrollHeight > MAX_COLLAPSED_HEIGHT + 30) {
            if (!wrapper.classList.contains('is-expanded')) {
                wrapper.style.maxHeight = MAX_COLLAPSED_HEIGHT + 'px';
    ...
    // Tính lại chiều cao sau khi các ảnh trong mô tả tải xong
    const descImages = content.querySelectorAll('img');
    ...
    window.toggleProductDescription = () => {
        const isExpanded = wrapper.classList.toggle('is-expanded');
        if (isExpanded) {
            wrapper.style.maxHeight = (content.scrollHeight + 50) + 'px';
            if (toggleText) toggleText.textContent = 'Thu gọn';
            if (toggleBtn) toggleBtn.classList.add('expanded');
        } else {
            wrapper.style.maxHeight = MAX_COLLAPSED_HEIGHT + 'px';
            if (toggleText) toggleText.textContent = 'Xem thêm mô tả';
