...
export async function generateAutomaticVouchers(userId, currentTier, totalSpent = 0) {
    if (!userId || userId === 'guest') return;
    if (!currentTier || currentTier.id === 'null') {
        currentTier = getMembershipTier(totalSpent);
    }
    ...
    try {
        ...
        // 1. Voucher thăng hạng (Tier-up Voucher) cho tất cả các mốc hạng đã đạt được
        for (const tier of MEMBERSHIP_TIERS) {
            if (tier.tierUpVoucher > 0 && totalSpent >= tier.min) {
                const tierCode = `UP${tier.id.toUpperCase()}${userId.substring(0, 5).toUpperCase()}`;
                const couponRef = doc(db, "coupons", tierCode);
                ...
                if (!couponSnap.exists()) {
                    await setDoc(couponRef, {
                        ...
                        assignedTo: userId,
                        conditions: `Mã ưu đãi đặc quyền khi đạt hạng ${tier.name}.\nÁp dụng cho mọi đơn hàng.\nChỉ sử dụng 1 lần duy nhất.`
                    });
                }
            }
        }
        ...
        if (userData) {
            ...
            if (currentTier.birthdayVoucher > 0 && userBday) {
                ...
                if (today.getMonth() === birthMonth) {
                    ...
                    if (!bdaySnap.exists()) {
                        const expiryDate = new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59);
                        await setDoc(bdayCouponRef, {
                            ...
                            isAutoGenerated: true,
                            assignedTo: userId,
                            conditions: `Mã ưu đãi mừng sinh nhật dành cho thành viên ${currentTier.name}.\nÁp dụng cho đơn hàng tối thiểu gấp đôi giá trị mã (${new Intl.NumberFormat('vi-VN').format(currentTier.birthdayVoucher * 2)}đ).\nHạn dùng trong tháng sinh nhật.`
                        });
                    }
                }
            }

            // 3. Voucher người thân
            if (currentTier.friendVoucher) {
                ...
                if (!friendSnap.exists()) {
                    await setDoc(friendCouponRef, {
                        ...
                        conditions: `Mã ưu đãi giảm 10% (tối đa 150.000đ) tặng người thân, bạn bè.\nChỉ áp dụng cho khách hàng mới chưa từng mua hàng tại Tiệm Nhà Gốm.`
                    });
                }
            }
        }
...
export async function logout() {
    try {
        ...
        if (window.updateMembershipPrices) window.updateMembershipPrices(null);
...
export function updateMembershipPrices(tier) {
    if (!tier) {
        try {
            ...
            if (tierStr) tier = JSON.parse(tierStr);
        } catch (e) { }
    }
    const containers = document.querySelectorAll('.dynamic-membership-price');
    ...
    containers.forEach(container => {
        const currentPrice = parseInt(container.getAttribute('data-price')) || 0;
        if (!currentPrice) return;
        const memPrice = Math.round(currentPrice * (1 - tier.discount / 100));
        const isDetail = Boolean(container.closest('.product-detail-info') || container.closest('#product-detail-container') || container.closest('.product-detail') || container.closest('.product-price-row'));
        if (isDetail) {
            container.innerHTML = `
                <div style="font-size: 0.9rem; display: flex; justify-content: space-between; align-items: center; padding: 6px 12px; border-radius: 8px; border: 1px solid #f0f0f0; background: #fafafa; margin-top: 10px; margin-bottom: 5px;">
            ...
            (async () => {
                try {
                    ...
                    try {
                        ...
                        sessionStorage.setItem('tng_current_tier', JSON.stringify(tier));
                        if (window.updateMembershipPrices) {
                            window.updateMembershipPrices(tier);
                        }
                        // Tự động cấp voucher thăng hạng, sinh nhật, người thân nếu đủ điều kiện
                        generateAutomaticVouchers(user.uid, tier, totalSpent);
                    } catch (e) {
                        console.error("Lỗi lấy hạng thành viên & tạo voucher:", e);
                    }
    ...
    try {
        ...
        const fixPaths = (html) => {
            ...
            return html
                ...
                .replace(/href="lookbook\//g, `href="${pathPrefix}lookbook/`)
